...
//            let imageBase64String = jpgData?.base64EncodedString(options: Data.Base64EncodingOptions.lineLength64Characters)
            ...
            let socket_PhotoItem = ChatMessage(type: "chat", sender: userid, senderName: userid, receiver: String(friendId) , content: imageBase64String!, messageType: "image")
    ...
    @IBAction func sendTextPressed(_ sender: UIButton) {
        ...
        //需更改成登入的使用者
        let socket_TextItem = ChatMessage(type: "chat", sender: userid, senderName: userid, receiver: String(friendId) , content: text, messageType: "text")
    ...
    public func websocketDidReceiveMessage(socket: WebSocketClient, text: String) {
        ...
        if message.messageType == "text" {
            let textDetail = "\(message.receiver): \(message.content)"
    ...
    public func websocketDidReceiveData(socket: WebSocketClient, data: Data) {
        ...
        let textDetail = "\(resultObject.sender):"
        let type : ChatSenderType = .fromOthers
